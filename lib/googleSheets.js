...
export async function saveLead(leadData) {
  try {
    ...
    const nameHeader = headers.find(h => headerMappings.name.some(m => h === m)) || 'Full Name';
    const emailHeader = headers.find(h => headerMappings.email.some(m => h === m)) || 'Email Address';
    const phoneHeader = headers.find(h => headerMappings.phone.some(m => h === m)) || 'Phone Number';
    const companyHeader = headers.find(h => headerMappings.company.some(m => h === m)) || 'Company Name';
    const serviceHeader = headers.find(h => headerMappings.service.some(m => h === m)) || 'Service Interested In';
    const budgetHeader = headers.find(h => headerMappings.budget.some(m => h === m)) || 'Budget Range';
    const timeHeader = headers.find(h => headerMappings.preferredTime.some(m => h === m)) || 'Preferred Contact Time';
    const messageHeader = headers.find(h => headerMappings.message.some(m => h === m)) || 'Additional Message';
