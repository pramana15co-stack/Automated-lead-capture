  ...
  try {
    ...
    if (!sheet) {
      sheet = await doc.addSheet({ 
        title: 'Leads',
        headerValues: ['Name', 'Email', 'Phone', 'Company', 'Service', 'Budget', 'PreferredTime', 'Message', 'Date', 'Timestamp']
      });
      ...
      const requiredHeaders = ['Name', 'Email', 'Phone', 'Company', 'Service', 'Budget', 'PreferredTime', 'Message', 'Date', 'Timestamp'];
      
      // If headers don't match, we'll add missing columns when saving
      if (headers.length < requiredHeaders.length) {
        logInfo('Sheet headers may need updating - will handle during save');
      }
    }
...
export async function getLeads() {
  try {
    ...
    const leads = rows.map(row => ({
      ...
      preferredTime: row.get('PreferredTime') || '',
