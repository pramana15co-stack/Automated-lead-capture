    ...
    sheet = doc.sheetsByTitle['Leads'] || doc.sheetsByTitle['Sheet1'] || doc.sheetsByIndex[0];
        ...
        if (!sheet) {
          ...
          sheet = await doc.addSheet({ 
            title: 'Leads',
            headerValues: ['Full Name', 'Email Address', 'Phone Number', 'Company Name', 'Service Interested In', 'Budget Range', 'Preferred Contact Time', 'Additional Message', 'Business Type', 'Contact Preference', 'Meta Ads', 'WhatsApp', 'Voice Assistant', 'Date', 'Timestamp', 'Status', 'Last Follow Up', 'Opt Out']
          });
          ...
          const requiredColumns = ['Business Type', 'Contact Preference', 'Meta Ads', 'WhatsApp', 'Voice Assistant', 'Date', 'Timestamp', 'Status', 'Last Follow Up', 'Opt Out'];
          let needsUpdate = false;
          let updatedHeaders = [...headers];
          
          for (const col of requiredColumns) {
            if (!headers.some(h => h === col || h.toLowerCase().includes(col.toLowerCase()))) {
...
export async function saveLead(leadData) {
  try {
    ...
    // Map to different possible header names - includes user's exact headers
    const headerMappings = {
      ...
      service: ['Service Interest', 'Service Interested In', 'Service', 'ServiceInterestedIn'],
      budget: ['Budget Range', 'Budget', 'BudgetRange'],
      preferredTime: ['Preferred Contact Time', 'Preferred Contact', 'PreferredTime', 'PreferredContactTime'],
      message: ['Additional Message', 'Message', 'AdditionalMessage'],
      businessType: ['Business Type', 'BusinessType'],
      contactPreference: ['Contact Preference', 'Preferred Contact Method', 'ContactPreference'],
      metaAds: ['Meta Ads', 'Meta Ads Setup', 'MetaAds'],
      whatsapp: ['WhatsApp', 'WhatsApp Service', 'WhatsAppService'],
      voiceAssistant: ['Voice Assistant', 'Voice Assistant Service', 'VoiceAssistant']
    };
    
    // Find matching header for each field - match user's exact headers
    const nameHeader = headers.find(h => headerMappings.name.some(m => h.includes(m) || m.includes(h))) || headers[0] || 'Full Name';
    const emailHeader = headers.find(h => headerMappings.email.some(m => h.includes(m) || m.includes(h))) || headers[1] || 'Email Address';
    const phoneHeader = headers.find(h => headerMappings.phone.some(m => h.includes(m) || m.includes(h))) || headers[2] || 'Phone Number';
    const companyHeader = headers.find(h => headerMappings.company.some(m => h.includes(m) || m.includes(h))) || headers[3] || 'Company Name';
    const serviceHeader = headers.find(h => headerMappings.service.some(m => h.includes(m) || m.includes(h))) || headers[4] || 'Service Interest';
    const budgetHeader = headers.find(h => headerMappings.budget.some(m => h.includes(m) || m.includes(h))) || headers[5] || 'Budget Range';
    const timeHeader = headers.find(h => headerMappings.preferredTime.some(m => h.includes(m) || m.includes(h))) || headers[6] || 'Preferred Contact Time';
    const messageHeader = headers.find(h => headerMappings.message.some(m => h.includes(m) || m.includes(h))) || headers[7] || 'Additional Message';
    const businessTypeHeader = headers.find(h => headerMappings.businessType.some(m => h.includes(m) || m.includes(h))) || 'Business Type';
    const contactPreferenceHeader = headers.find(h => headerMappings.contactPreference.some(m => h.includes(m) || m.includes(h))) || 'Contact Preference';
    const metaAdsHeader = headers.find(h => headerMappings.metaAds.some(m => h.includes(m) || m.includes(h))) || 'Meta Ads';
    const whatsappHeader = headers.find(h => headerMappings.whatsapp.some(m => h.includes(m) || m.includes(h))) || 'WhatsApp';
    const voiceAssistantHeader = headers.find(h => headerMappings.voiceAssistant.some(m => h.includes(m) || m.includes(h))) || 'Voice Assistant';
    const dateHeader = headers.find(h => h === 'Date' || h.toLowerCase().includes('date')) || 'Date';
    const timestampHeader = headers.find(h => h === 'Timestamp' || h.toLowerCase().includes('timestamp')) || 'Timestamp';
    ...
    rowData[statusHeader] = 'Not Contacted'; // Initial status
    rowData[lastFollowUpHeader] = '';
    rowData[optOutHeader] = 'No';
