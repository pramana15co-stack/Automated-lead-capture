...
const templates = {
  ...
  ownerNotification: (data) => ({
        ...
        <div class="container">
          ...
          <div class="content">
            <div class="lead-info">
              ...
              ${data.contactPreference ? `<p><strong>📞 Preferred Contact:</strong> <strong style="color: #2563eb; font-size: 1.1em;">${data.contactPreference}</strong></p>` : ''}
              ${data.budget ? `<p><strong>Budget:</strong> ${data.budget}</p>` : ''}
              ${data.preferredTime ? `<p><strong>Preferred Time:</strong> ${data.preferredTime}</p>` : ''}
              ${data.message ? `<p><strong>Message:</strong> ${data.message}</p>` : ''}
              ${data.services ? `
                <div style="margin-top: 15px; padding: 15px; background: #eff6ff; border-radius: 8px; border-left: 4px solid #2563eb;">
                  <p style="margin: 0 0 10px 0; font-weight: 700; color: #1e40af;">📦 Additional Services Requested:</p>
                  ${data.services.metaAds ? '<p style="margin: 5px 0;">✅ <strong>Meta Ads Setup</strong> - $500 one-time</p>' : ''}
                  ${data.services.whatsapp ? '<p style="margin: 5px 0;">✅ <strong>WhatsApp Notification System</strong> - $400 setup + $50/month</p>' : ''}
                  ${data.services.voiceAssistant ? '<p style="margin: 5px 0;">✅ <strong>Voice Assistant System</strong> - $800 setup + $80/month</p>' : ''}
                  ${!data.services.metaAds && !data.services.whatsapp && !data.services.voiceAssistant ? '<p style="margin: 5px 0; color: #6b7280;">No additional services selected</p>' : ''}
                </div>
              ` : ''}
    ...
    text: `
      ...
      ${data.contactPreference ? `📞 Preferred Contact: ${data.contactPreference}\n` : ''}${data.budget ? `Budget: ${data.budget}\n` : ''}${data.preferredTime ? `Preferred Time: ${data.preferredTime}\n` : ''}${data.message ? `Message: ${data.message}\n` : ''}
      ${data.services ? `
      📦 Additional Services Requested:
      ${data.services.metaAds ? '✅ Meta Ads Setup - $500 one-time\n' : ''}${data.services.whatsapp ? '✅ WhatsApp Notification System - $400 setup + $50/month\n' : ''}${data.services.voiceAssistant ? '✅ Voice Assistant System - $800 setup + $80/month\n' : ''}${!data.services.metaAds && !data.services.whatsapp && !data.services.voiceAssistant ? 'No additional services selected\n' : ''}
