...
export function validateLeadData(data) {
  ...
  // Budget is optional
  if (data.budget) {
    validated.budget = String(data.budget).trim();
  }

  // Preferred time is optional
  if (data.preferredTime) {
    validated.preferredTime = String(data.preferredTime).trim();
  }
  ...
  if (data.businessType) {
    validated.businessType = String(data.businessType).trim();
  }

  // Contact preference is optional but recommended
  if (data.contactPreference) {
    validated.contactPreference = String(data.contactPreference).trim();
  }

  // Services (add-ons) are optional
  if (data.services && typeof data.services === 'object') {
    validated.services = {
      metaAds: Boolean(data.services.metaAds),
      whatsapp: Boolean(data.services.whatsapp),
      voiceAssistant: Boolean(data.services.voiceAssistant)
    };
  }
